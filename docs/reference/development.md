...
未選択checkをN/Aとして保存しません。失敗や新しい影響が見つかった場合は、根拠に対応する一つの軸だけを拡張し、決定的な成功条件を満たした時点で停止します。

## リポジトリ固有規則の維持

導入先のbranch、commit、ownership、security、build、test規則を維持します。このportable契約はbranch戦略、release marker、reconciliation方式、承認者を新たに規定しません。導入先のmachine-readable policyがある場合はそれを正本として検査し、説明文へ同じ現在状態を複製しません。
...
変更ごとの計画、implementation log、test report、CI生ログを重複保存しません。一時実行状態が必要な場合だけ`.devflow/run/`を使用し、Git管理しません。regulated案件だけが必要な`work/<id>/`とlifecycle evidenceを持ちます。
