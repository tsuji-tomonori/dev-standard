...
5. [adapter契約](adapter-contract.md)のschema v3で`.dev-standard/design.json`を作成する。全適用要件をcapabilityへ集合として完全対応させ、全必要surfaceをrequired capabilityへ接続する。APIは[6帳票profile](api-documents.md)と実operation inventoryを接続する。
...
旧`designflow.py`、`qualityflow.py`、`portable_python.py`は配布しない。既存のschema v1/v2のAPI契約はv3へ移す。旧generatorを使用中なら導入先で所有する実装へ移し、解析意味と既存公開契約を回帰検査してからmanifestへ接続する。旧生成物を新しい所有rootに残すと検査は失敗する。過去の配置を維持する場合は導入先profileのlayoutを明示する。
...
最初に主参照の固定SHA・67要件の個別受入条件・適用外を要件正本へ反映する。次にoperation所有とendpoint→業務処理→query/外部portの依存、型・契約・sample・応答組立を整理する。公開HTTP、認可順、transaction/rollback/retry/冪等性を維持し、配置だけの変更で互換を推定しない。

その後に内容を生成するadapter、正負例を実行するconformance command、CRUD model v2の全資源台帳とgroup別exportを接続する。6帳票の実内容を確認し、新規意味検査と非破壊check・二重生成・関連業務テストを実行する。[conformance.md](conformance.md)の契約を満たすまで完了にしない。先にgeneratorが固定のpassや空のunsupportedを出すようにして検査を通さない。

旧v2を調査する場合だけ`check_design.py --legacy-layout-only`を明示できる。結果は`reference_conformance.status: not-checked-legacy`であり、新規導入の検証入口には接続しない。v3へ移した後はこのoptionを拒否する。新しいschemaだけを宣言して古いadapter・CRUD形式・定数reportを使う移行は完了しない。
