...
def openapi_docs(document: dict[str, Any]) -> tuple[str, str]:
    ...
    catalog = ["# API catalog", "", "| Method | Path | Operation ID | Summary | Requirement IDs |", "|---|---|---|---|---|"]
...
def sql_docs(root: Path) -> tuple[str, str]:
    ...
    for path in sorted(root.rglob("*.sql")):
        ...
        for index, statement in enumerate(statements, 1):
            ...
            queries.append({"id": f"{relative}-{index}", "file": path.relative_to(root).as_posix(), "operation": operation, "target": target or "-", "tables": sorted(tables)})
...
def command_templates(kind: str) -> tuple[str, str]:
    ...
    if kind == "fastapi":
        arguments = "fastapi --source-root <source-root> --openapi <openapi.json> --sql-root <sql-root> --out <output>"
    elif kind == "cdk":
...
def write_bundle(out: Path, files: dict[str, str], sources: list[tuple[str, Path]], kind: str) -> None:
    ...
    try:
        ...
        for name, content in files.items():
            if not name.endswith(".gen.md"):
                raise DesignError(f"generated Markdown must end with .gen.md: {name}")
            (temporary / name).write_text(banner + content, encoding="utf-8", newline="\n")
...
def generate_fastapi(args: argparse.Namespace, out: Path) -> None:
    ...
    sources = (
        [(f"source/{path.relative_to(args.source_root).as_posix()}", path) for path in routers + function_files]
        + [(f"openapi/{args.openapi.name}", args.openapi)]
        + [(f"sql/{path.relative_to(args.sql_root).as_posix()}", path) for path in sql_files]
    )
    write_bundle(out, {"SEQUENCES.gen.md": render_sequences(operations), "API_CATALOG.gen.md": api, "INTERFACES.gen.md": interfaces, "CRUD.gen.md": crud, "QUERY_OBJECTS.gen.md": queries}, sources, "fastapi")
