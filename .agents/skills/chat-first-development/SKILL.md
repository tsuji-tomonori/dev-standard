...
変更した挙動・操作に関係する既存の利用手順だけを更新する。継続更新する文書は既存索引に利用者・目的・更新条件を記し、生成物は正本から再生成する。不要な文書・未使用templateは呼出元を調べて整理する。時点の報告は日付付きの履歴へ分け、現在仕様として更新し続けない。文書保守だけのSkill・承認工程・変更報告書を導入先へ増やさない。
