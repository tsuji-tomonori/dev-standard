...
3. Separate one-off noise from recurring process failures. Read `references/improvement-policy.md`. Aggregate underestimated task classes, repeated reads of the same digest/range, repeated searches, ineffective capability escalation, unnecessary subagents, and expansion count from verification failure to root cause.
