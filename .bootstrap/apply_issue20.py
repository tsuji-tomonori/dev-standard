...
ORIGINAL_BASE_SHA = os.environ["ORIGINAL_BASE_SHA"]
...
def merge_text_file(relative: str) -> None:
    print(f"phase: three-way merge {relative}")
    path = ROOT / relative
    ours = OURS / relative
    base = subprocess.run(
        ["git", "show", f"{ORIGINAL_BASE_SHA}:{relative}"],
        cwd=ROOT,
        capture_output=True,
        check=False,
    )
    if base.returncode:
        print(base.stderr.decode(errors="replace"), file=sys.stderr)
        raise SystemExit(f"cannot read original base file: {relative}")
    with tempfile.NamedTemporaryFile() as base_file:
        base_file.write(base.stdout)
        base_file.flush()
        result = subprocess.run(
            ["git", "merge-file", "-p", str(path), base_file.name, str(ours)],
            cwd=ROOT,
            capture_output=True,
            check=False,
        )
    if result.returncode:
        ...
        raise SystemExit(f"three-way merge conflict: {relative}")
...
def main() -> None:
    ...
    merge_text_file("README.md")
    merge_text_file("docs/reference/development.md")
    merge_governance()
    merge_requirements()
