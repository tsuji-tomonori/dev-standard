...
const destination = process.env.EVIDENCE_E2E_OUTPUT || await mkdtemp(path.join(tmpdir(), 'dev-standard-evidence-e2e-'));
const generated = spawnSync(process.env.PYTHON || '.venv/bin/python', ['tests/build_evidence_hierarchy_fixture.py', destination], { encoding: 'utf8', env: { ...process.env, PYTHONPATH: process.cwd() } });
...
try {
  ...
  async function scenario(id, name, action) {
    ...
    const steps = [await step(id, 'Given', '実装由来6帳票とCRUDを持つ品質portalを開く')];
