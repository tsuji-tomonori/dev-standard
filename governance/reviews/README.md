...
- `as_built_adoption`時の採用scopeと適用除外
- 選択されたcheck ID
...
`as_built_standard_change`は、as-built標準の要件正本、標準文書、check catalog、generator contract、配布定義、trace契約を定義または変更する場合に使用し、`FAST-024`で正本間の整合を検査する。generator生成対象や定量閾値も変える場合は、独立して`generated_change`または`quality_threshold_change`を設定する。

`as_built_adoption`は、repositoryがas-built標準を具体的なartifact・code scopeへ採用する、または適用scopeを拡張する場合だけ使用する。schema v2では`impact_details.as_built_adoption.scope`を1件以上記録し、適用除外を`exclusions`へ明示する。coverage、test構造、解析可能な実装規約、外部report集約は`FAST-019`〜`FAST-021`、`FAST-023`で評価する。標準を変更しただけではこれらを選択しない。

`e2e_change`はSQLを変えずE2E scenarioまたは状態assertだけを変更する場合に使用する。`quality_threshold_change`は定量閾値またはlinter delegation設定を変更する場合に使用し、標準・設定・委譲先の一致checkをRisk-selectedとして選択する。

schema v2を新規reviewの既定とする。schema v1の既存reviewは後方互換のため受理し、v1に存在しない`as_built_standard_change`をfalseとしてselectorを評価する。既存reviewを新しいcatalog digestへ合わせて書き換えない。
